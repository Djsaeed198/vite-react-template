...
// Connection test routine as requested by the skill guidelines
...
// Direct popups are preferred inside the build iframe ecosystem
export async function signInWithGoogle() {
  try {
    const result = await signInWithPopup(auth, googleProvider);
    return result.user;
  } catch (err) {
    console.error("Google Auth popup registration error:", err);
    throw err;
  }
}
